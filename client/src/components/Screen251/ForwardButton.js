...
const ForwardButton = ({ selectedArtist }) => {
  ...
  const handleClick = () => {
    // Send GET request to http://127.0.0.1:5000/recommend with the selected artist
    if (!selectedArtist) {
      console.log("No artist selected. Cannot proceed.");
      return;
    }
    console.log("Selected Artist:", selectedArtist);
    fetch("https://25-pnyx-clyvyprrn-vidhip30s-projects.vercel.app/recommend", {
      method: "POST",
      headers: {
  ...
  return (
    <Link to={selectedArtist ? "/25-pnyx/generating-playlist" : "#"} onClick={handleClick}>
